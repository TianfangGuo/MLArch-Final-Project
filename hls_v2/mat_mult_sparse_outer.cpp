...
void mat_mult(
    ...
    uint8_t  A_row_ind_buf[MAT_DIM * MAT_DIM];
	uint8_t  A_col_ptr_buf[MAT_DIM + 1];
	uint32_t A_val_buf[MAT_DIM * MAT_DIM];

	uint8_t  B_row_ptr_buf[MAT_DIM + 1];
	uint8_t  B_col_ind_buf[MAT_DIM * MAT_DIM];
	uint32_t B_val_buf[MAT_DIM * MAT_DIM];

	uint8_t  C_row_ind_buf[MAT_DIM * MAT_DIM];
	uint8_t  C_col_ind_buf[MAT_DIM * MAT_DIM];
	...
	load_A_val: for (uint16_t i = 0; i < A_nnz; i++) {
	#pragma HLS pipeline
		A_row_ind_buf[i] = A_row_ind[i];
		A_val_buf[i] = A_val[i];
	}

	load_B_val: for (uint16_t i = 0; i < B_nnz; i++) {
	#pragma HLS pipeline
		B_col_ind_buf[i] = B_col_ind[i];
		B_val_buf[i] = B_val[i];
	}

	uint32_t C_temp[MAT_DIM][MAT_DIM] = {0};

	// Perform the matrix multiplication (outer product)
	compute_k: for (uint8_t k = 0; k < MAT_DIM; k++) {
		uint8_t A_start = A_col_ptr_buf[k];
		uint8_t A_end = A_col_ptr_buf[k + 1];
		uint8_t A_len = A_end - A_start;
		uint8_t B_start = B_row_ptr_buf[k];
		uint8_t B_end = B_row_ptr_buf[k + 1];
		uint8_t B_len = B_end - B_start;
	...
	coo: for (uint8_t i = 0; i < MAT_DIM; i++) {
		coo_j: for (uint8_t j = 0; j < MAT_DIM; j++) {
		#pragma HLS pipeline
			if (C_temp[i][j] != 0) {
	...
	// Store the matrix
	*C_nnz = C_nnz_buf;
	store_C: for (uint16_t i = 0; i < C_nnz_buf; i++) {
	#pragma HLS pipeline
